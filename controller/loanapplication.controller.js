...
module.exports.createnewloan_application = (req,res) =>{
 ...
 try {
    ...
    if(!errrorResponse.isEmpty()){
        ...
        DB.query("SELECT * FROM loan_application WHERE user_id = ? AND loan_status = ?", [user_id, "active"], (e, user)=>{
            if(e){
               ...
               if(user.length > 0){
                res.status(400).json({message: "you are currently on loan"})
                   ...
                   DB.query ("INSERT INTO loan_application(user_id, loan_amount, loan_term, loan_purpose, loan_status) VALUES (?,?,?,?,?)",[user_id, loan_amount, loan_term, loan_purpose, loan_status ], (err, _) =>{
                       if(err){
                           console.log(err)
                               res.status(500).json({message: "unable to apply for new loan"})
                       }else{
...
module.exports.updateloan_applicationstatus = (req, res) =>{
  ...
  try {
    if (!errrorResponse.isEmpty()){
      ...
      DB.query('UPDATE loan_application SET  loan_status = ? WHERE loan_application_id = ?', [loan_status, loan_application_id], (e, _)=>{
        if(e){
...
module.exports.getAllapplication =(req, res) =>{
    try {
        DB.query("SELECT * FROM loan_application", (err, loan_application) =>{
            if(err){
...
module.exports.deleteapplication = (req, res) =>{
  ...
  try {
    if (!loan_application_id){
      ...
      DB.query("DELETE FROM loan_application WHERE loan_application_id = ?", [loan_application_id], (er, _) =>{
        if(er){
          res.status(500).json({message: "unable to delete loan_application"})
        }else{
